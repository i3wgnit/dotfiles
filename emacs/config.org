                ...
                (lambda ()
                  (concat (if TeX-PDF-mode "-pdf " "")
                          (cond
                           ...
                           (t "")))))
              TeX-expand-list)
  (cl-pushnew '("LatexMK" "latexmk %(-PDF)%S%(mode) %(file-line-error) %(extraopts) %t"
                TeX-run-TeX nil t)
              TeX-command-list))
#+END_SRC
...
(use-package tex
  :ensure auctex
  :defines
  (TeX-expand-list TeX-command-list)
  :functions
  (LaTeX-math-mode TeX-source-correlate-mode LaTeX-newline)
